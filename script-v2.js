    ...
    const heroDots  = document.querySelectorAll('#heroRefDots .hrd');
        ...
        function loadSlide(idx) {
            ...
            if (s && s.banner) {
                ...
                card.style.cursor = (s && s.href) ? 'pointer' : '';
            }
            heroDots.forEach((d, i) => d.classList.toggle('active', i === idx));
            current = idx;
        }

            // Click the excerpt card to follow its slide href (if any)
            card.addEventListener('click', (e) => {
                // Don't navigate if clicking on arrow buttons
                if (e.target.classList.contains('hero-ref-arr') || e.target.closest('.hero-ref-arr')) return;
        ...
        heroPrev && heroPrev.addEventListener('click', () => {
            console.log('heroPrev clicked — current=', current);
            stopAuto();
            const idx = (current >= 0) ? (current - 1 + total) % total : 0;
            goTo(idx, false);
            startAuto();
        });
        heroNext && heroNext.addEventListener('click', () => {
            console.log('heroNext clicked — current=', current);
            stopAuto();
            const idx = (current >= 0) ? (current + 1) % total : 0;
            goTo(idx, true);
            startAuto();
        });
